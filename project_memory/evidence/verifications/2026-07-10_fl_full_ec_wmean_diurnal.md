# 2026-07-10 FL 全量 w_mean 日变化图

## 目的

可视化 `FL` 全量产品的平均垂直风速 `w_mean` 日变化。

## 脚本

- `D:\00 博士阶段\99 Project\06 EA\scripts\plot_fl_full_ec_wmean_diurnal.R`
...
- 不回原始高频重算
- 直接复用已交付 `FL` 全量 `30 min EC` 结果表中的 `w_mean`
- 所有 `source_group` pooled 后，按半小时计算中位数与 `25-75%` 四分位带
- 三种方法 `no_rotation / dr / PF_8bin_2ensemble` 同图对比

## 输出

- 图：`E:\Dataset_Level1\Flares\EC_ecpreproc\figures_diurnal\FL_full_ec_wmean_diurnal.png`
- 作图表：`E:\Dataset_Level1\Flares\EC_ecpreproc\FL_full_ec_wmean_diurnal_plot_data.csv`
- 说明：`E:\Dataset_Level1\Flares\EC_ecpreproc\FL_full_ec_wmean_diurnal_summary.txt`
