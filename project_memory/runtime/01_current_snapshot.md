...
需要保持的解释边界是：这批新增单程只满足放宽轨道 `15-255 m` 下的 `geometry_complete == TRUE`，可用于解释运行记录里的 coverage 缺口，但不能替代 strict pass、EC-valid 样本或质量守恒主结果统计口径。[推断: 基于 strict coverage 与 relaxed geometry coverage 的用途区分]

## 2026-07-06 最新同步：raw/PF F_adv 日变化图色带

`E:\FL_MASSBALANCE\202308` 几何放宽批次和 `E:\FL_MASSBALANCE` 全量批次的 raw/PF `F_adv` 分组日变化图已经补充覆盖范围色带。两套图均包含 raw、raw detrended、PF、PF detrended 四张，且色带统一表示每个半小时、每个 `broad/closed` 分组在日期层面的 `25-75%` 范围。 [已核验: project_memory/evidence/verifications/2026-07-06_fl_fadv_diurnal_ribbons_raw_pf.md]

这次更新只改变可视化，不改变质量守恒计算、闭合分类或去趋势口径。全量 raw 图使用 `2275` 个 pass 和 `122` 个日期，全量 PF 图使用 `2589` 个 pass 和 `123` 个日期；202308 几何放宽 raw 图使用 `1288` 个 pass 和 `74` 个日期，PF 图使用 `1637` 个 pass 和 `74` 个日期。 [已核验: project_memory/evidence/verifications/2026-07-06_fl_fadv_diurnal_ribbons_raw_pf.md]

## 2026-07-06 最新同步：W3 两固定塔自然年 NEE 估算已独立成线
...
`E:\FL_MASSBALANCE\202308` 下已经完成几何放宽 `15-255 m` 完整单程批次的 raw/PF `F_adv` 质量守恒计算。该批次请求 `1823` 个 `geometry_complete == TRUE` 单程，raw 与 PF 两版均实际计算 `1777` 个；raw 版 mixed-sign 为 `1628` 个，PF 版 mixed-sign 为 `1734` 个。 [已核验: project_memory/evidence/verifications/2026-07-03_fl_202308_geometry_relaxed_raw_pf_fadv.md]

当前 raw 日变化图显示 08:30 有正向爬升，但诊断表明这不是多数日期稳定结构，而是少数日期抬高均值；08:30 的 `extreme_forced` class 加权均值接近 `0`，因此不应简单写成 lambda 强制闭合造成。PF 后同一批样本的 08:30 转为明显负谷值，且 `extreme_forced` 从 raw 的 `340` 降至 `97`、`single_sign_down` 从 `121` 降至 `0`，说明 PF 版本更少受 raw `Uz` 单符号下沉和极端闭合类别支配。 [已核验: project_memory/evidence/verifications/2026-07-03_fl_202308_geometry_relaxed_raw_pf_fadv.md] [推断：基于 raw/PF 类别数量与日变化诊断整理]

## 2026-07-02 最新同步：FL F_lambda(x) 空间 profile 解释边界
当前全量 `F_lambda(x)` 空间 profile 的纵轴应按质量守恒修正后的移动剖面 CO2 垂直输送诊断量理解，即 `w_lambda * CO2 * rho_air`，不是 `MT/CVT` 的 EC covariance 通量，也不是 30 min 长期日变化均值。因此图中几百到几千 `umol m-2 s-1` 的量级主要反映 `w_lambda` 与空气 CO2 摩尔密度的乘积，特别是负 `w` 被较大 `lambda` 放大后的结果。 [已核验: project_memory/evidence/verifications/2026-07-02_fl_lambda_spatial_profile_interpretation.md]

当前解释优先级是：`broad_closed` 作为主解释层，显示沿 `0-245 m` 轨道的位置依赖相对弱；全数据纳入 `numerically_closed` 与 `extreme_forced` 后，前 `0-50 m` 的向下输送信号更明显，但该结构同时带有更强 `lambda` 放大风险，应作为高风险样本空间诊断，而不直接写成稳健主结论。 [已核验: project_memory/evidence/verifications/2026-07-02_fl_lambda_spatial_profile_interpretation.md] [推断：基于当前 profile 数值范围和 closure_class 分层目的整理]

## 2026-07-02 最新同步：MT/CVT 与 FL 30 min 日变化对比

`MT/CVT` 全量 `sector_pf` 通量已经和 `FL` 质量守恒 half-hour closure mean 接到同一张 30 min 长期日变化对比图中。固定塔使用 `MT=38316`、`CVT=21447` 条记录，日期数分别为 `883` 和 `493`；参与比较的时间范围为 `MT: 2023-06-19 10:28:13` 至 `2026-06-22 13:30:00`，`CVT: 2024-11-01 00:30:00` 至 `2026-05-10 15:00:00`。 [已核验: project_memory/evidence/verifications/2026-07-02_mt_cvt_fl_30min_diurnal_comparison.md]

本次图形口径已经简化为 `MT`、`CVT`、`FL_broad` 和 `FL_closed` 四条曲线；`MT/CVT` 不用圆点表达数据量，`FL` 仍保留点大小表达参与均值的日期数。主输出位于 `E:\FL_MASSBALANCE\figures\mt_pf_fl_30min_diurnal`，并已复制到 `D:\00 博士阶段\博一\05 Project\com_rotation\results\mt_pf_fl_30min_diurnal`。 [已核验: project_memory/evidence/verifications/2026-07-02_mt_cvt_fl_30min_diurnal_comparison.md]

## 2026-07-02 最新同步
W2 晨间 peak 的事件检测口径已经固定到一个可复现版本。当前定义不再要求日出后 `co2_mean` 逐点单调下降或上升，而是要求它从 `sunrise_ref` 到 `pre_min_time` 整体下降、再从 `pre_min_time` 到 `peak_time` 整体上升；`amp_ppm` 则固定为两时刻廓线均值之差。[来源: 用户当前对话 2026-07-02] [已核验: project_memory/evidence/verifications/2026-07-02_w2_morning_peak_fixed_rule_overall_decline_rise.md]

按这个固定口径重跑后，`CVT` 的 `usable_days = 199`、`>=5 ppm = 83`、`>=10 ppm = 42`，`MT` 的 `usable_days = 182`、`>=5 ppm = 72`、`>=10 ppm = 34`；双塔按天为 `event_5ppm_any = 96`、`event_5ppm_both = 59`、`event_10ppm_any = 46`、`event_10ppm_both = 30`。当前最小下一步不再是继续改事件定义，而是对 `amp > 5 ppm` 的站点日做人工复核。[来源: 用户当前对话 2026-07-02] [已核验: project_memory/evidence/verifications/2026-07-02_w2_morning_peak_fixed_rule_overall_decline_rise.md]

## 2026-07-01 最新同步

固定塔全量 after-PF 通量已经推进到可复查的数据产品阶段。`MT` 当前主表为 `E:\Dataset_Level1\MT\EC\Flux_ecprecproc_afterPF\MT_flux_sector_pf.csv`，采用 `sector_pf` 默认口径，并已覆盖 2023-07 至 2023-12、2024-01/03 的时间读取修复重跑结果；2024-03 仍保留少量首边界启发式相位提醒，需要后续按原始文件边界复核。 [已核验: project_memory/evidence/verifications/2026-07-01_full_pf_flux_progress_locations.md]

`CVT` 已在 `E:\Dataset_Level1\CVT\EC\PF` 完成全量 `sector_pf` 通量计算和均值可视化。主表 `CVT_flux_sector_pf.csv` 为 `21447` 行，覆盖 `2024-11-01 00:30` 至 `2026-05-10 15:00`，无重复 timestamp；图表目录为 `E:\Dataset_Level1\CVT\EC\PF\figures_flux_means`。 [已核验: E:\Dataset_Level1\CVT\EC\PF\CVT_flux_sector_pf_validation_summary.csv] [已核验: E:\Dataset_Level1\CVT\EC\PF\CVT_sector_pf_flux_mean_visualization_summary.txt]

## 2026-06-29 最新同步

W2 晨间 peak 全年数据整理已经形成第一版可用基线。登记表口径下 `AP_PAIR` 为 `365` 天完整，`MET_PAIR` 为 `362` 天完整，`EC_PAIR` 与 `ALL_SIX` 均为 `293` 天完整；实际 Level0 AP `.dat` 解析口径更严格，双塔 AP 同日 `full-day` 为 `327` 天，非双塔完整为 `38` 天。 [来源: 用户当前对话 2026-06-29] [已记录: project_memory/evidence/discussions/2026-06-29_w2_annual_peak_screening_results.md]

当前可进入晨间 peak 初筛的站点日为 `CVT 342` 天、`MT 339` 天，其中 `CVT` 短波日出缺失 `9` 天。暂定幅度阈值下，`CVT >=5 ppm` 有 `154` 天、`CVT >=10 ppm` 有 `90` 天；`MT >=5 ppm` 有 `91` 天、`MT >=10 ppm` 有 `47` 天。推断：W2 已经从“生成按日覆盖清单”推进到“已有覆盖分母和初筛候选池”，下一步应冻结事件识别规则并检查双塔同步、领先关系和气象状态分布。 [来源: 用户当前对话 2026-06-29] [推断：基于 W2 当前完成标准整理]

## 2026-06-28 最新同步

当前 `E:\FL_MASSBALANCE` 中与质量守恒主结果配套的分层诊断图已经补齐到 `broad_closed` 与 `numerically_closed` 两类 mixed-sign 单程。新的筛选热图样本为 `2589` 个单程、`123` 个有效日期和 `15` 个有效月份；月度图、合并图与观测簇拆分图统一写入 `E:\FL_MASSBALANCE\figures\monthly_transport_heatbands`，文件名带后缀 `_broad_numerically_closed`，用于和保留全量 mixed-sign 的旧图并存。 [已核验: project_memory/evidence/verifications/2026-06-28_fl_mass_balance_closure_mean_flux_and_filtered_heatmaps.md]

当前 closure-class 通量均值的正式汇总口径也已经固定：只使用 mixed-sign 单程，先在每个 `date × hour_bin × lambda_closure_class` 内按单程与该小时的 `overlap_sec` 对 `F_lambda_pf_umol_m2_s` 加权平均，再跨日期做等权平均；`month × hour × closure_class` 热图沿用同样的“先时长加权、再日期等权”层级。对应图件与 CSV 已输出到 `E:\FL_MASSBALANCE\figures\lambda_diagnostics`。 [已核验: E:\FL_MASSBALANCE\plot_fl_mass_balance_closure_mean_flux.R] [已核验: E:\FL_MASSBALANCE\figures\lambda_diagnostics\FL_mass_balance_closure_mean_flux_summary.txt]

这套均值图当前显示 `broad_closed` 与 `numerically_closed` 的最强负向小时均值都在午前后，分别为 `broad_closed@12=-14.80` 和 `numerically_closed@12=-14.00`；热图色标使用聚合均值绝对值 `P98` 裁剪，当前上限为 `25`。这些结果仍应作为质量守恒修正后的诊断输送量解释，主解释优先 `broad_closed`，`numerically_closed` 作为敏感性对照，不直接升级为最终生态系统 CO2 通量。 [已核验: E:\FL_MASSBALANCE\figures\lambda_diagnostics\FL_mass_balance_closure_mean_flux_summary.txt] [推断：基于当前 closure-class 分层规则和图件用途整理]
...
晨间 peak 主线的下一最小步，是在已经得到全年覆盖口径和暂定阈值候选数量后，冻结自动事件识别草案，明确 `peak`、`pre-min`、`duration`、`post-peak decline`、`profile switch` 和无 peak 对照日的定义；随后检查 `CVT/MT` 候选的同步或领先关系，并筛查 `20-30` 个三站事件日的气象状态分布。 [来源: 用户当前对话 2026-06-29] [已记录: project_memory/evidence/discussions/2026-06-29_w2_annual_peak_screening_results.md] [推断：基于 W2 当前完成标准整理]
