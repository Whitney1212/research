...
- 每个项目保留独立 `project_memory`，不要合并成一个总文件。 [推断：基于 REgov 设计]
- REgov 只维护跨项目索引、理论卡、知识源策略、workstream 可视化和输出治理。 [推断：基于 REgov 设计]
- 跨项目联系必须保留来源边界；从 `04 Lee` 借鉴到当前项目的内容统一标为 `推断：`，除非有当前项目证据直接支持。 [推断：基于 project memory 证据规则]
- 当前项目和 `04 Lee` 都是 REgov 历史背景网络的一部分，但普通回应不默认展开全部背景；只有当用户要求跨项目比较、机制溯源、方法继承或历史决策追踪时，才进入跨项目读取。 [来源: 用户当前对话 2026-05-27] [推断：基于 REgov 分层读取规则]
