...
- 数据资产覆盖情况以 `E:\Dataset_RAW\数据存入-处理记录.xlsx` 为主要索引，以 `E:\Dataset_RAW\20260428数据甘特图.png` 为当前原始数据覆盖图示；这些原始数据可以按具体问题随时处理和分析。 [来源: 用户当前对话 2026-05-28] [已核验: E:\Dataset_RAW\数据存入-处理记录.xlsx] [已核验: E:\Dataset_RAW\20260428数据甘特图.png]
...
- 但在 2026-06-06 之后，这条 `09:00-11:00` 事件归因线在 REgov 中应调整为复杂地形通量修正框架下的复现与支撑案例，而不是仓库级主线。它的主要用途是帮助判断储存、平流、通风和局地环流何时干扰 EC 解释。 [来源: 用户当前对话 2026-06-06] [已核验: project_memory/evidence/verifications/2026-06-06_regov_mainline_reset.md]
