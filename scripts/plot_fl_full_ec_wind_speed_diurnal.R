...
read_case <- function(row) {
  ...
  need <- c("u_mean", "v_mean", "timestamp", "date")
  miss <- setdiff(need, names(dt))
  if (length(miss) > 0L) {
    stop("Missing columns in ", row$file[[1]], ": ", paste(miss, collapse = ", "), call. = FALSE)
  }
  dt[, `:=`(
    u_mean = as.numeric(u_mean),
    v_mean = as.numeric(v_mean)
  )]
  dt[, wind_speed_mean := sqrt(u_mean^2 + v_mean^2)]
  dt[, `:=`(
...
build_diurnal_stats <- function(plan_dt) {
  ...
  stats <- dt[, .(
    ...
    q25 = as.numeric(stats::quantile(wind_speed_mean, probs = 0.25, na.rm = TRUE, names = FALSE)),
    median_value = stats::median(wind_speed_mean, na.rm = TRUE),
    q75 = as.numeric(stats::quantile(wind_speed_mean, probs = 0.75, na.rm = TRUE, names = FALSE))
  ), by = .(rotation_method, method_label, half_hour_bin)]
...
self_check <- function(stats_dt) {
  ...
  stopifnot(all(stats_dt$q25 <= stats_dt$median_value))
  stopifnot(all(stats_dt$median_value <= stats_dt$q75))
...
plot_csv <- file.path(out_root, "FL_full_ec_wind_speed_diurnal_plot_data.csv")
plot_png <- file.path(figure_dir, "FL_full_ec_wind_speed_diurnal.png")
summary_txt <- file.path(out_root, "FL_full_ec_wind_speed_diurnal_summary.txt")
...
p <- ggplot(
  ...
  labs(
    title = "FL full-data mean wind speed by rotation method",
    ...
    y = expression("Horizontal mean-wind speed (" * m ~ s^-1 * ")"),
    caption = "Wind speed is computed as sqrt(u_mean^2 + v_mean^2) from delivered FL full EC 30 min outputs."
  ) +
  theme_regov(base_size = 13)

ggsave(plot_png, p, width = 12, height = 7.2, dpi = 300)
fwrite(copy(stats_dt)[, method_label := as.character(method_label)], plot_csv)
...
writeLines(
  c(
    "FL full EC wind speed diurnal summary",
    ...
    "",
    "Notes:",
    "- Uses delivered FL full EC 30 min outputs only; does not recompute wind from raw data.",
    "- Wind speed = sqrt(u_mean^2 + v_mean^2).",
...
message("Wrote FL full EC wind speed diurnal outputs to: ", figure_dir)
