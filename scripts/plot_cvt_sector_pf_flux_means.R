...
input_file <- "E:/Dataset_Level1/CVT/EC/PF/CVT_flux_sector_pf.csv"
output_dir <- "E:/Dataset_Level1/CVT/EC/PF"
...
long <- melt(dt, measure.vars = vars, variable.name = "variable",
...
fwrite(overall, file.path(output_dir, "CVT_sector_pf_flux_mean_overall.csv"))

hourly <- long[, mean_se(value), by = .(variable, variable_label, hour)]
hourly[, ci95 := 1.96 * se]
fwrite(hourly, file.path(output_dir, "CVT_sector_pf_flux_mean_by_hour.csv"))

month_hour <- long[, mean_se(value), by = .(variable, variable_label, year_month, hour)]
fwrite(month_hour, file.path(output_dir, "CVT_sector_pf_flux_mean_by_month_hour.csv"))
...
p_overall <- ggplot(overall, aes(x = variable_label, y = mean)) +
  ...
  labs(
    title = "CVT sector PF full-period mean fluxes",
    x = NULL,
    y = "Mean value",
    caption = "All available rows in CVT_flux_sector_pf.csv; error bars are +/- 1.96 SE."
  ) +
  theme_regov() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
ggsave(file.path(figure_dir, "CVT_sector_pf_flux_overall_means.png"), p_overall, width = 12, height = 8, dpi = 300)
...
p_hour <- ggplot(hourly, aes(x = hour, y = mean)) +
  ...
  labs(
    title = "CVT sector PF mean diurnal flux pattern",
    x = "Hour of day (Asia/Shanghai)",
    y = "Mean value",
    caption = "All available rows; ribbon is +/- 1.96 SE by hour."
  ) +
  theme_regov()
ggsave(file.path(figure_dir, "CVT_sector_pf_flux_mean_diurnal.png"), p_hour, width = 12, height = 10, dpi = 300)
...
for (v in heatmap_vars) {
  ...
  p <- ggplot(sub, aes(x = year_month, y = hour, fill = mean)) +
    ...
    labs(
      title = paste0("CVT sector PF mean ", var_labels[[v]], " by month and hour"),
      x = "Year-month",
  ...
  ggsave(file.path(figure_dir, paste0("CVT_sector_pf_flux_mean_month_hour_", v, ".png")),
         p, width = 15, height = 7.5, dpi = 300)
}

summary_txt <- file.path(output_dir, "CVT_sector_pf_flux_mean_visualization_summary.txt")
writeLines(c(
  "CVT sector_pf full-period flux mean visualization",
  ...
  "",
  "CSV outputs:",
  "- CVT_sector_pf_flux_mean_overall.csv",
  "- CVT_sector_pf_flux_mean_by_hour.csv",
  "- CVT_sector_pf_flux_mean_by_month_hour.csv",
  "",
  "Figure outputs:",
  "- figures_flux_means/CVT_sector_pf_flux_overall_means.png",
  "- figures_flux_means/CVT_sector_pf_flux_mean_diurnal.png",
  "- figures_flux_means/CVT_sector_pf_flux_mean_month_hour_co2_flux.png",
  "- figures_flux_means/CVT_sector_pf_flux_mean_month_hour_H.png",
  "- figures_flux_means/CVT_sector_pf_flux_mean_month_hour_LE.png",
  "- figures_flux_means/CVT_sector_pf_flux_mean_month_hour_u_star.png",
  "",
  "Notes:",
...
message("Wrote CVT sector_pf flux mean visualizations to: ", figure_dir)
