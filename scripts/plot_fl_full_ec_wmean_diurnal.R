...
build_diurnal_stats <- function(plan_dt) {
  ...
  stats <- dt[, .(
    ...
    q25 = as.numeric(stats::quantile(w_mean, probs = 0.25, na.rm = TRUE, names = FALSE)),
    median_value = stats::median(w_mean, na.rm = TRUE),
    q75 = as.numeric(stats::quantile(w_mean, probs = 0.75, na.rm = TRUE, names = FALSE))
...
plot_csv <- file.path(out_root, "FL_full_ec_wmean_diurnal_plot_data.csv")
plot_png <- file.path(figure_dir, "FL_full_ec_wmean_diurnal.png")
summary_txt <- file.path(out_root, "FL_full_ec_wmean_diurnal_summary.txt")
...
p <- ggplot(
  ...
  labs(
    title = "FL full-data mean vertical wind by rotation method",
    ...
    caption = "w_mean is taken directly from delivered FL full EC 30 min outputs."
  ) +
...
writeLines(
  c(
    "FL full EC w_mean diurnal summary",
    ...
    "",
    "Notes:",
    "- Uses delivered FL full EC 30 min outputs only; does not recompute w_mean from raw data.",
    "- w_mean is taken directly from the delivered tables.",
...
message("Wrote FL full EC w_mean diurnal outputs to: ", figure_dir)
