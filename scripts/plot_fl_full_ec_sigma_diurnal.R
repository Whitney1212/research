...
read_case <- function(row) {
  ...
  need <- c("w_sd", "scalar_sd", "timestamp", "date")
  ...
  dt_long <- rbindlist(list(
    dt[is.finite(w_sd), .(
      ...
      rotation_method,
      method_label,
      variable = "Wind fluctuation",
      value = w_sd
    )],
    dt[is.finite(scalar_sd), .(
      date,
      half_hour_bin,
      source_group,
      rotation_method,
      method_label,
      variable = "CO2 fluctuation",
      value = scalar_sd
    )]
  ))
  dt_long
...
build_diurnal_stats <- function(plan_dt) {
  ...
  ), by = .(variable, rotation_method, method_label, half_hour_bin)]
  stats[, method_label := factor(method_label, levels = unname(method_labels[method_levels]))]
  stats[, variable := factor(variable, levels = c("Wind fluctuation", "CO2 fluctuation"))]
  setorder(stats, variable, rotation_method, half_hour_bin)
  stats
}
...
plot_csv <- file.path(out_root, "FL_full_ec_sigma_diurnal_plot_data.csv")
plot_png <- file.path(figure_dir, "FL_full_ec_sigma_diurnal.png")
summary_txt <- file.path(out_root, "FL_full_ec_sigma_diurnal_summary.txt")
...
p <- ggplot(
  ...
  labs(
    title = "FL full-data diurnal fluctuations by rotation method",
    ...
    y = NULL,
    caption = paste(
      "Wind fluctuation uses w_sd (30 min sigma_w); CO2 fluctuation uses scalar_sd.",
      "Inputs are the delivered FL full EC outputs under E:/Dataset_Level1/Flares/EC_ecpreproc."
    )
  ) +
...
summary_lines <- c(
  "FL full EC sigma diurnal summary",
  ...
  "- Wind fluctuation = w_sd (30 min sigma_w).",
  "- CO2 fluctuation = scalar_sd.",
...
), by = .(variable, method_label)]

writeLines(
  c(
    summary_lines,
    "",
    "Coverage by variable and method:",
    apply(coverage, 1, function(x) {
      sprintf(
        "- %s / %s: bins=%s, n_dates range=%s-%s, n_windows range=%s-%s, pooled source_groups=%s",
        x[["variable"]], x[["method_label"]], x[["n_half_hour_bins"]],
...
message("Wrote FL full EC sigma diurnal outputs to: ", figure_dir)
